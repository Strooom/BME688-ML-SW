...
/* Exchange a byte */
static BYTE xchg_spi(BYTE dataToTransmit) {
    BYTE receivedData;
    HAL_SPI_TransmitReceive(&hspi1, &dataToTransmit, &receivedData, 1, 50);
    return receivedData;
}

/* Receive multiple byte */
static void rcvr_spi_multi(BYTE *receiveBuffer, UINT length) {
    for (UINT i = 0; i < length; i++) {
        *(receiveBuffer + i) = xchg_spi(0xFF);
    }
}

#if _USE_WRITE
/* Send multiple byte */
static void xmit_spi_multi(const BYTE *transmitBuffer, UINT length) {
    HAL_SPI_Transmit(&hspi1, transmitBuffer, length, HAL_MAX_DELAY);
}
#endif
...
/* Wait for card ready                                                   */
/*-----------------------------------------------------------------------*/
static int wait_ready(UINT timeOutInMs) {
    BYTE response;
    uint32_t timerStart            = HAL_GetTick();
    uint32_t waitSpiTimerTickDelay = (uint32_t)timeOutInMs;

    while ((HAL_GetTick() - timerStart) < waitSpiTimerTickDelay) {
...
static void despiselect(void) {
    HAL_GPIO_WritePin(CS_SDCARD_GPIO_Port, CS_SDCARD_Pin, GPIO_PIN_SET); /* Set CS# high */
...
{
    HAL_GPIO_WritePin(CS_SDCARD_GPIO_Port, CS_SDCARD_Pin, GPIO_PIN_RESET); /* Set CS# low */
...
inline DSTATUS USER_SPI_initialize(
    ...
    MODIFY_REG(hspi1.Instance->CR1, SPI_BAUDRATEPRESCALER_256, SPI_BAUDRATEPRESCALER_128);        //    Set SCLK = slow, approx 280 KBits/s
    ...
    if (ty) {                                                                                       /* OK */
        MODIFY_REG(hspi1.Instance->CR1, SPI_BAUDRATEPRESCALER_256, SPI_BAUDRATEPRESCALER_8);        // Set SCLK = fast, approx 4.5 MBits/s
