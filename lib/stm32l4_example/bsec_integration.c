 ...
 *
 * @param[in]    sample_rate         BSEC_SAMPLE_RATE_SCAN
 *
 ...
 * @param[in]   dev         		pointer to the sensor communication and inventory details structure
 *
 * @return      zero if successful, negative otherwise
 */
return_values_init bsec_iot_init(float sample_rate, float temperature_offset, bme68x_write_fptr_t bus_write, bme68x_read_fptr_t bus_read, sleep_fct sleep, state_load_fct state_load, config_load_fct config_load, struct bme68x_dev dev) {
    ...
    bme68x_g.read  = bme68x_i2c_read;
    bme68x_g.write = bme68x_i2c_write;
#elif defined(USE_SPI_INTERFACE)
    /* Fixed SPI configuration */
    bme68x_g.intf  = BME68X_SPI_INTF;
    bme68x_g.read  = SensorAPI_SPIx_Read;
    bme68x_g.write = SensorAPI_SPIx_Write;
#endif
...
#elif defined(BME68X_OUTPUT_GAS_IAQ)
static bsec_library_return_t bme68x_bsec_process_data(bsec_input_t *bsec_inputs, uint8_t num_bsec_inputs, output_iaq_ready_fct output_ready) {
    uint8_t num_bsec_outputs = 0;
    uint8_t index            = 0;
    ...
    if (num_bsec_inputs > 0) {
        ...
        /* Pass the extracted outputs to the user provided output_ready() function. */
        output_ready(timestamp, iaq, iaq_accuracy, static_iaq, co2_equivalent, breath_voc_equivalent, raw_pressure, raw_temp, raw_humidity, raw_gas, raw_gas_index, bsec_status);
    }
    return bsec_status;
}
#endif
...
#elif defined(BME68X_OUTPUT_GAS_IAQ)
uint8_t processData(int64_t currTimeNs, struct bme68x_data data, int32_t bsec_process_data, output_iaq_ready_fct output_ready)
#endif
{
...
#elif defined(BME68X_OUTPUT_GAS_IAQ)
void bsec_iot_loop(sleep_fct sleep, get_timestamp_us_fct get_timestamp_us, output_iaq_ready_fct output_ready,
                   state_save_fct state_save, uint32_t save_intvl)
#endif
{
    /* Timestamp variables */
    int64_t time_stamp = 0;
    volatile uint32_t tick_before, tick_after;
